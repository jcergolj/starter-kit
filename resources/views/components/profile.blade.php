...
<div {{ $attributes->merge(['class' => 'group flex items-center rounded-lg has-data-[circle=true]:rounded-full p-1 hover:bg-base-300']) }}>
    <div class="shrink-0">
        <svg
            version="1.1"
    ...
    @if ($name)
        <span class="text-base-200 group-hover:text-base-300 mx-2 truncate text-sm font-medium"> {{ $name }} </span>
    @endif
</div>
