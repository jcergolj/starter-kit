        ...
        <x-page-card class="my-6">
            @if ($user->hasEnabledTwoFactorAuthentication())
                <div class="relative flex flex-col items-start justify-start space-y-4 rounded-xl">
                    ...
                    <div class="card bg-base-100 border-base-content/10 w-full border">
                        <div class="card-body space-y-2">
                            ...
                            <div class="card-actions">
                                <x-turbo::frame id="recovery-codes" class="block w-full">
                                    <a
                                        href="{{ route('settings.recovery-codes.edit') }}"
                                        class="btn btn-soft space-x-1"
                                    >
                    ...
                    <form
                        action="{{ route('settings.two-factor.destroy') }}"
                        data-turbo-confirm="{{ __('Are you sure you want to disable it?') }}"
                        method="post"
                    >
                        @csrf
                        @method('DELETE')
            ...
            @else
                <x-turbo::frame
                    ...
                    data-action="turbo:frame-missing->frame#breakoutWhenMissing"
                    ...
                    >
                        ...
                        <div>
                            <x-form.button.primary
                                type="submit"
                                class="flex w-full items-center space-x-0.5"
                                data-bridge--form-target="submit"
                                data-bridge-title="{{ __('Enable') }}"
                            >
