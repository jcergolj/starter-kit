<x-layouts.auth :title="__('Authentication code')">
    <div class="flex flex-col gap-6">
        <input
            type="checkbox"
            name="recovery_toggle"
            id="recovery_toggle"
            @if ($errors->has('recovery_code')) checked @endif
            class="peer hidden"
        />
        ...
        <form
            id="two-factor-code-form"
            action="{{ route('two-factor.login.store') }}"
            method="post"
            class="flex flex-col gap-6 peer-checked:hidden"
        ...
        >
            ...
            <div class="space-x-0.5 text-center text-sm leading-5">
                ...
                <div class="inline cursor-pointer font-medium underline opacity-80">
                    ...
                    >{{ __('login using a recovery code') }}</label>
                </div>
            </div>
        </form>

        <form
            id="two-factor-recovery-form"
            action="{{ route('two-factor.login.store') }}"
            method="post"
            class="hidden flex-col gap-6 peer-checked:flex"
            data-turbo-action="replace"
        >
            @csrf
            ...
            <div>
                ...
                <x-form.text-input
                    ...
                    :data-error="$errors->has('recover_code')"
                />
            ...
            <div class="space-x-0.5 text-center text-sm leading-5">
                ...
                <div class="inline cursor-pointer font-medium underline opacity-80">
                    <label
                        ...
                        role="button"
                        class="cursor-pointer"
                    >{{ __('login using an authentication code') }}</label>
                </div>
            </div>
        </form>
    </div>
