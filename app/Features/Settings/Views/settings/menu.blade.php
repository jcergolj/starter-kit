<x-layouts.app :title="__('Profile & Settings')">
    <section class="mx-auto w-full space-y-6 lg:max-w-xl">
        ...
        <x-menu>
            ...
            @if (\Laravel\Fortify\Features::canManageTwoFactorAuthentication())
                <x-menu.link icon="lock-closed" :href="route('settings.two-factor.edit')">
                    {{ __('Two-factor authentication') }}</x-menu.link>
            @endif
        ...
        <x-menu> <x-menu.button icon="chevron-left" form="settings-logout" type="submit">{{ __('Logout') }}</x-menu>
        </x-menu>
    </section>
