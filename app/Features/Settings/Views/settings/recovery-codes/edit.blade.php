        ...
        <x-page-card class="my-6">
            @if ($user->hasEnabledTwoFactorAuthentication())
                <div class="relative flex flex-col items-start justify-start space-y-4 rounded-xl">
                    ...
                    <div class="card bg-base-100 border-base-200 w-full border">
                        <div class="card-body space-y-2">
                            ...
                            <div class="card-actions">
                                <x-turbo::frame id="recovery-codes" target="_top">
                                    ...
                                    <div
                                        class="mt-4 grid gap-1 rounded-lg bg-zinc-100 p-4 font-mono text-sm dark:bg-white/5"
                                        role="list"
                                    ...
                                    <div class="mt-4">
                                        <p class="text-base-content/50 text-xs">
                                            {{ __('If you run out of codes or suspect they have been compromised, you can generate a new set. This will invalidate all existing codes.') }}
                                        </p>

                                        <form
                                            class="mt-2 block"
                                            action="{{ route('settings.recovery-codes.update') }}"
                                            method="post"
                                            data-turbo-confirm="{{ __('Are you sure you want to regenerate the codes?') }}"
                                        >
                                            @csrf
                                            @method('PUT')
